...
class TestAccountReports(common.TransactionCase):
    ...
    def test_00_financial_reports(self):
        for report in self.env['account.financial.html.report'].search([]):
            context_data = self.env['account.report.context.common'].return_context('account.financial.html.report', {},
                                                                                    report.id)
    ...
    def test_02_followup_reports(self):
        ...
        self.env['account.report.context.followup'].create(
            {'partner_id': self.env.ref('base.res_partner_2').id}).get_html()
