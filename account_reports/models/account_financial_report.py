...
class ReportAccountFinancialReport(models.Model):
    ...
    cash_basis = fields.Boolean('Use cash basis',
                                help='if true, report will always use cash basis, if false, user can choose from filter inside the report')
    analytic = fields.Boolean('Allow analytic filter', help='display the analytic filter')
    hierarchy_option = fields.Boolean('Enable the hiarchy option',
...
class AccountFinancialReportLine(models.Model):
    ...
    def _get_sum(self, currency_table, financial_report, field_names=None):
        ...
        if self.domain:
            ...
            res = self.with_context(strict_range=strict_range, date_from=date_from, date_to=date_to)._compute_line(
                currency_table, financial_report, group_by=self.groupby, domain=self.domain)
        return res
    ...
    def get_balance(self, linesDict, currency_table, financial_report, field_names=None):
        ...
        res = dict((fn, 0.0) for fn in field_names)
        c = FormulaContext(self.env['account.financial.html.report.line'], linesDict, currency_table, financial_report,
                           self)
        if self.formulas:
        ...
        if self.domain and self.groupby and self.show_domain != 'never':
            ...
            if formulas:
                for key in results:
                    ...
                    c['sum_if_pos'] = FormulaLine(results[key]['balance'] >= 0.0 and results[key] or {'balance': 0.0},
                                                  currency_table, financial_report, type='not_computed')
                    c['sum_if_neg'] = FormulaLine(results[key]['balance'] <= 0.0 and results[key] or {'balance': 0.0},
                                                  currency_table, financial_report, type='not_computed')
