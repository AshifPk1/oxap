...
class report_account_consolidated_journal(models.AbstractModel):
    ...
    def get_columns_name(self, options):
        columns = [{'name': _('Journal Name (Code)')}, {'name': _('Debit'), 'class': 'number'},
    ...
    def get_line_total_per_month(self, options, current_company, results):
        ...
        if dates:
            ...
            for date in sorted(dates):
                ...
                sum_debit = self.format_value(sum([int(r['debit']) for r in results if
                                                   (r['month'] == month and r['yyyy'] == year) and r[
                                                       'company_id'] == current_company]))
                sum_credit = self.format_value(sum([int(r['credit']) for r in results if
                                                    (r['month'] == month and r['yyyy'] == year) and r[
                                                        'company_id'] == current_company]))
                sum_balance = self.format_value(sum([int(r['balance']) for r in results if
                                                     (r['month'] == month and r['yyyy'] == year) and r[
                                                         'company_id'] == current_company]))
                vals = {
