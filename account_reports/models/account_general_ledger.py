...
class report_account_general_ledger(models.AbstractModel):
    ...
    def group_by_account_id(self, options, line_id):
        ...
        initial_bal_results = self.with_context(date_to=initial_bal_date_to.strftime('%Y-%m-%d')).do_query(options,
                                                                                                           line_id)
        ...
        if unaffected_earnings_xml_ref:
            ...
            unaffected_earnings_results = self.with_context(date_to=last_day_previous_fy.strftime('%Y-%m-%d'),
                                                            date_from=False).do_query_unaffected_earnings(options,
                                                                                                          line_id)
        ...
        if not unaffected_earnings_line and not float_is_zero(unaffected_earnings_results['balance'],
            ...
            unaffected_earnings_account = self.env['account.account'].search([
                ...
                ('company_id', 'in', self.env.context.get('company_ids', []))
            ], limit=1)
