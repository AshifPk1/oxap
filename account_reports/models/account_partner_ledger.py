...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def do_query(self, options, line_id):
        ...
        tables, where_clause, where_params = self.env['account.move.line'].sudo()._query_get(
    ...
    def group_by_partner_id(self, options, line_id):
        ...
        for partner_id, result in results.items():
            ...
            if not context.get('print_mode'):
                ...
                partners[partner]['lines'] = self.env['account.move.line'].sudo().search(domain, order='date', limit=81)
            else:
                partners[partner]['lines'] = self.env['account.move.line'].sudo().search(domain, order='date')
        return partners
