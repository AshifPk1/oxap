...
class AccountReport(models.AbstractModel):
    ...
    def _build_options(self, previous_options=None):
        ...
        if self.env.user.id in group_multi_company.users.ids:
            ...
            options['multi_company'] = [
                {'id': c.id, 'name': c.name, 'selected': True if c.id == self.env.user.company_id.id else False} for c
                in self.env.user.company_ids]
                ...
                if key == 'date' or key == 'comparison':
                    ...
                    if previous_options[key].get('filter', 'custom') != 'custom':
                        ...
                        company_fiscalyear_dates = self.env.user.company_id.compute_fiscalyear_dates(
                            datetime.strptime(previous_options[key]['date'], DEFAULT_SERVER_DATE_FORMAT))
    ...
    def get_report_informations(self, options):
        ...
        info = {'options': options,
                ...
                'searchview_html': self.env['ir.ui.view'].render_template(
                    self.get_templates().get('search_template', 'account_report.search_template'),
        ...
        for line in lines:
            ...
            while group_key:
                ...
                hierarchy_list[group_key]['values']['columns'] = [sum(x) for x in pycompat.izip(
                    hierarchy_list[group_key]['values']['columns'], [c['no_format_name'] for c in columns])]
    ...
    def format_date(self, dt_to, dt_from, options, dt_filter='date'):
        ...
        return _('From %s <br/> to  %s') % (format_date(self.env, dt_from.strftime(DEFAULT_SERVER_DATE_FORMAT)),
            ...
            for index in range(0, options['comparison'].get('number_period', 1)):
                ...
                if dt_from:
                    vals = {'date_from': dt_from.strftime(DEFAULT_SERVER_DATE_FORMAT),
                            'date_to': dt_to.strftime(DEFAULT_SERVER_DATE_FORMAT), 'string': display_value}
                else:
