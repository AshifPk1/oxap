...
class report_account_followup_report(models.AbstractModel):
    ...
    def get_lines(self, options, line_id=None):
        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                ...
                line_num += 1
                columns = [format_date(self.env, aml.date, lang_code=lang_code), date_due,
            ...
            lines.append({
                ...
                'unfoldable': False,
                'level': 0,
                'columns': [{'name': v} for v in [''] * (2 if self.env.context.get('print_mode') else 4) + [
    ...
    def get_default_summary(self, options):
        return self.env.user.company_id.overdue_msg or self.env['res.company'].default_get(['overdue_msg'])[
            'overdue_msg']
    ...
    def get_report_manager(self, options):
        ...
        if not existing_manager:
            existing_manager = self.env['account.report.manager'].create({'report_name': 'account.followup.report',
                                                                          'company_id': selected_companies and
                                                                                        selected_companies[0] or False,
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        if self.env.context.get('followup_line_id'):
            ...
            report = {'name': self.get_report_name(),
                      ...
                      'followup_line': self.env['account_followup.followup.line'].browse(
                          self.env.context.get('followup_line_id')), }
            additional_context['report'] = report
        return super(report_account_followup_report, self).get_html(options, line_id=line_id,
                                                                    additional_context=additional_context)

    def get_pdf(self, options, minimal_layout=True):
        return super(report_account_followup_report, self.with_context(keep_summary=True)).get_pdf(options,
    ...
    def get_report_value(self, partner, options):
        ...
        return {'name': self.get_report_name(), 'summary': self.get_report_manager(options).summary,
                'company_name': self.env.user.company_id.name}
    ...
    def get_history(self, partner):
        return self.env['mail.message'].search(
            [('subtype_id', '=', self.env.ref('account_reports.followup_logged_action').id),
             ('id', 'in', partner.message_ids.ids)], limit=5)
